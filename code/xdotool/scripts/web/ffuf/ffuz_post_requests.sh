...
: '
ffuf: : do a post request with given post data, wordlist and filter
'
...
execute_command "ffuf -w ${DIR_WORDLIST} -u '${WEBSITE}' -X POST -H 'Content-Type: application/x-www-form-urlencoded' -d '${POST_DATA}' -fr '${FILTER_TEXT}' -maxtime-job 40"
create_new_line
