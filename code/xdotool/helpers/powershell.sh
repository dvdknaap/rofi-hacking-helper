...
wget_windows_download() {
    local url="$1"
    local location="$2"

    if [[ -z "$1" ||  -z "$2" ]]; then
        paste_command 'Usage: wget_windows_download "url" "location"'
        exit 1
    fi
...
wget_windows_upload_file() {
    ...
    wget_windows_download "http://${DOWNLOAD_IP}:${PYTHON_HTTP_PORT}/${file}" "${FILE_LOCATION}"
